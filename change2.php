...
echo $exm;
include("connection.php");
$qry="SELECT sid,subject1,class from invoice where subject1 <='30' AND class='$cls'";
								...
								if(mysqli_num_rows($s)>0){
									...
									<tr>
										...
										<td> <?php echo $row["class"]; ?>
											<input type ='hidden' value=' <?php echo $row["class"] ?> '/>
										</td>
										</td>
										</tr>";
										<?php 
...
$exm=$_POST["c1"];
$qry="SELECT sid,subject2,class from invoice where subject2 <='30' AND class='$cls'";
$s=mysqli_query($con,$qry);
echo"<table border ='1' cellspacing=3 width=70% height=80% align='center' style=margin:90px; bgcolor='white'>
<tr><th colspan=10><font size=15 color=black >extra class</font></th></tr>
<tr><td>sid</td><td>subject2</td><td>class</td></tr>";
$counter=0;
								if(mysqli_num_rows($s)>0){
